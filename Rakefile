...
namespace :formula do
  ...
  desc "Watch Compass project in `/assets/formula` for changes"
  task :watch do
    `bundle exec compass watch ./assets/formula`
  end
  
  desc "Compile Compass project in `/assets/formula`"
  task :compile do
    `bundle exec compass compile ./assets/formula`
  end
